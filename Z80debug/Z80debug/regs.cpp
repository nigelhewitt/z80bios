...
void doReg(HWND hDlg, UINT id, const char* fmt, int value)
{
	char temp[10];
	sprintf_s(temp, sizeof temp, fmt, value);
	SetDlgItemText(hDlg, id, temp);
}
void doFlag(HWND hDlg, UINT id, bool v)
{
	CheckDlgButton(hDlg, id, v ? BST_CHECKED : BST_UNCHECKED);
}
INT_PTR RegsProc(HWND hDlg, UINT wMessage, WPARAM wParam,  LPARAM lParam)
{
	char temp[10];
...
void unpackRegs(const char* text, REGS* regs)
{
	...
	int index = 0;
	regs->SP  = unpackWORD(text, index);
	regs->IY  = unpackWORD(text, index);
	regs->IX  = unpackWORD(text, index);
	regs->HL  = unpackWORD(text, index);
	regs->DE  = unpackWORD(text, index);
	regs->BC  = unpackWORD(text, index);
	regs->A   = unpackBYTE(text, index);
	regs->F   = unpackBYTE(text, index);
	regs->PC  = unpackWORD(text, index);
	regs->AA  = unpackBYTE(text, index);
	regs->FA  = unpackBYTE(text, index);
	regs->BCA = unpackWORD(text, index);
	regs->DEA = unpackWORD(text, index);
	regs->HLA = unpackWORD(text, index);
...
void unpackMAP(const char* text, REGS* regs)
{
	...
	int index = 0;
	for(int i=0; i<4; regs->PAGE[i++]  = unpackBYTE(text, index));
	SendMessage(hRegs, WM_USER, 0, 0);	// update
}
void ShowRegs()
{
	if(hRegs==nullptr)
		hRegs = CreateDialog(hInstance, MAKEINTRESOURCE(IDD_REGS), hFrame, RegsProc);
